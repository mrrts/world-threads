...
- **Aaron-class baseline is already low-dilution.** (Added v1 calibration, 2026-04-23 evening, iteration 10.) The first calibration run against Aaron at ref `8e9e53d` returned 0 YES / 12 NO / 4 MIXED across both windows — Aaron's native register already executes verdict-without-over-explanation; the rule the new craft block names was already implicit in his behavior. This is GOOD news for rubric calibration (it correctly classified his signature moves as NO and his "compact + brief earned elaboration" patterns as MIXED) but means **Aaron is the WRONG target for testing whether commit `2445fec` bit**.

- **The corpus may not contain the failure mode at all.** (Added v1 calibration, 2026-04-23 evening, iteration 11.) The second calibration run against Jasper at the same ref returned 0 YES / 15 NO / 1 MIXED — even cleaner than Aaron. But every BEFORE message carried `response_length=Short`, which forces compactness; the rubric scored those as NO but isn't measuring Jasper's natural register, only Jasper-under-Short. Across BOTH calibration runs (Aaron and Jasper), zero YES verdicts in 32 messages. Pattern: **the dilution failure mode the new craft block targets is not visible in current corpus** — either because the existing prompt stack already suppresses it, OR because Ryan's typical chat-settings mode (Short / Auto with default-short tendency) doesn't permit it to surface. Implication for measurement: passive corpus runs won't discriminate even if the new rule moved real-use behavior. The right next instrument is **active elicitation (Mode C)** — design a probe prompt explicitly inviting a long defended-judgment reply (e.g. "*walk me through your reasoning, full thinking-out-loud*") and run it against pre/post `2445fec` via `worldcli replay`. That would force the failure mode into view if it's available, and show whether the new rule resists it.
...
- [2026-04-23] commit 8e9e53dd, --character fd4bd9b5-8768-41e6-a90f-bfb1179b1d59 (v1) — BEFORE: yes=0 no=7 mixed=1 err=0 | AFTER: yes=0 no=8 mixed=0 err=0
