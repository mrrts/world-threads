...
- **The chooser is the Codex UI contract.** The turn does not end with prose cleanup or a soft close; it ends with exactly four numbered next moves (`1.`-`4.`), so the user can answer in one keystroke.
- **Selection triggers immediate execution (project law).** When the user picks a chooser option, that selection immediately starts the next concrete work unit in the same flow (no promise-only pause turn).
