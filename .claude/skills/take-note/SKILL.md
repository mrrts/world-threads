...
Commit `reports/OBSERVATIONS.md` updates per the project's standing commit-and-push autonomy (see CLAUDE.md and the `feedback_commit_push_autonomy` memory entry — reports always ship). Commit message should name the observation title and the mode picked, e.g. *"observation: characters feel more vividly themselves (Mode 3 — noted)"*.
