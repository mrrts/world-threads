...
Keep chooser cardinality fixed at 4 options. When branching is sparse, include adjacent productive branches (execute, inspect, tighten, user-defined move) rather than collapsing to minimal fallback sets. The run's arc closes when the reflection lands AND the chooser is offered — not when the reflection alone is written.
