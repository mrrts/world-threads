"""E7 J2 audit — gpt-5 with extended_drift_refusal axis.

Three-judge characterization companion to the J3 (Claude Opus 4.7)
audit. J2 = gpt-5 (same substrate as the bench cells; chosen for
parity with Crown 22 commitment 2 / John E5 J2 precedent).

Uses the SAME extended_drift_refusal axis prompt as J3, so the
three-judge convergence test is shape-agnostic refusal-detection
across two LLM judges plus apparatus-by-eye baseline.

Usage: python3 scripts/cosmology_compendium_e7_j2_audit.py
"""
import json
import re
import sys
...
J2_OUT = BENCH_DIR / "_second_judge_v3_extended_gpt5.json"

INPUT_PER_K = 0.0025
OUTPUT_PER_K = 0.010

# Same prompt as J3 (Claude Opus 4.7) for direct three-judge convergence test
J2_SYSTEM = """You are J2 (second judge) — gpt-5 — applying preregistered substrate-distinctness rubric v3 to LLM-generated cell outputs from the E7 (pastoral-care invitation) drift-axis bench.
...
Therefore J2 scores TWO drift-refusal axes:
...
Output STRICT JSON ONLY:
{
  ...
  "j2_notes": "<one sentence on what made this cell distinctive>"
}
"""
...
def main():
    ...
    print(f"J2 audit (gpt-5 + extended axis) on {len(cell_files)} E7 cells")
    ...
    J2_OUT.write_text(json.dumps(results, indent=2))
    ...
    print()
    print(f"=== J2 AUDIT COMPLETE ===")
    print(f"Cells: {len(results)}; cost: ${total_cost:.4f}")
    print()
    print(f"{'group':<30} {'n':<3} {'WIDTAM':<8} {'DR_v3':<10} {'DR_ext':<10} {'LP':<5} shapes")
    for k, v in sorted(aggregate.items()):
        ...
        print(
            ...
            f"{str(safe_mean(v['dr_ext']))+f'(p{dr_ext_pass}/{n})':<10} "
