...
enum Cmd {
    ...
    /// payload file against the live character row.
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.cmd {
        ...
        Cmd::AuditCharacterIdentity { character_id, emit_payload, compare_to } => {
            cmd_audit_character_identity(&r, &character_id, emit_payload, compare_to.as_deref())
        }
        ...
        Cmd::Ask { character_id, message, session, model, confirm_cost, question_summary, no_anchor, world_description_override, omit_craft_rule, synthetic_history, include_documentary_rules, inject_file, inject_before, inject_after, section_order, end_seal, no_end_seal, fence_pipeline, group_chat, with_momentstamp, momentstamp_override, short_mode, cosmology_override, mission_formula_override } => {
            ...
            if let Some(gc_id) = group_chat {
                let effective_message = if short_mode {
                    format!(
                        "{}\n\n[SHORT-MODE CONTRACT: reply in <=20 words; use a warm invitational opener, then one explicit concrete next move with a 10-minute bound (imperative or direct question); no stage business unless physically required.]",
                        message
                    )
                } else {
                    message.clone()
...
fn cmd_audit_character_identity(
    ...
    let result = if let Some(path) = compare_to {
