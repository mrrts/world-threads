...
3. Voicing density follows mood — count **notes only** (rests are silence, not compositional weight). Per 4-bar phrase across all 4 tracks combined, ORDERED so density INCREASES from heavy → patient → bright: heavy/burden → 4-8 notes (sparse, generously punctuated by rests); patient/wisdom → 7-14 notes (more woven, less negative space); bright/grace → 12-22 notes (densest, often with noise rhythm activity). The increase across mood classes must be visible in the note count, not just claimed in mood_descriptor. Under continuation (prev_phrase present), bright moods following bright should reach the higher end of the range; bright moods escalating from heavy should clearly add notes vs. the previous phrase. Track INSTRUMENTS may evolve too — bring in sawtooth/sine for warmer moods, lean on pulse for sharper.
