...
fn synthesis_system_prompt(character: &Character, user_display_name: &str) -> String {
...
fn synthesis_user_prompt(
    ...
    if !kept_records.is_empty() {
        let body: Vec<String> = kept_records
            .iter()
            .take(20)
    ...
    if !recent_journals.is_empty() {
        let body: Vec<String> = recent_journals
            .iter()
            .rev()
            .take(5)
...
fn collect_recent_excerpts(
    ...
    let Ok(thread) = get_thread_for_character(conn, character_id) else {
        return Vec::new();
    };
    let Ok(mut msgs) = list_messages(conn, &thread.thread_id, limit) else {
        return Vec::new();
    };
...
pub async fn refresh_relational_stance(
    ...
    let (character, user_profile, kept_records, recent_journals, recent_excerpts, world_day_now) = {
        ...
        let kept_records: Vec<KeptRecord> = stmt
            .query_map(rusqlite::params![character_id, character.world_id], |r| {
                Ok(KeptRecord {
                    kept_id: r.get(0)?,
        ...
        let recent_journals = list_journal_entries(&conn, &character_id, 5).unwrap_or_default();
