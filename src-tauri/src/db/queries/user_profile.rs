...
pub struct UserProfile {
    ...
    /// Friendly-prose translation of the Unicode-math derivation
    /// above. Surfaced in the UI so non-technical users (per the
    /// Maggie baseline) can see and refine "how characters see me"
    /// without needing LaTeX or math knowledge. Auto-generated by
    /// the two-output synthesis pipeline whenever derived_formula
    /// is generated via the UI wizard. NULL when only the
    /// background auto-refresh has populated derived_formula
...
pub fn get_user_profile(conn: &Connection, world_id: &str) -> Result<UserProfile, rusqlite::Error> {
    conn.query_row(
        "SELECT world_id, display_name, description, facts, boundaries, avatar_file, updated_at, derived_formula, derived_summary FROM user_profiles WHERE world_id = ?1",
        params![world_id],
        |row| Ok(UserProfile {
            world_id: row.get(0)?,
