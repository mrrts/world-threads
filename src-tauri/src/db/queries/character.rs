...
pub fn list_archived_characters(
    conn: &Connection,
    world_id: &str,
) -> Result<Vec<Character>, rusqlite::Error> {
    let mut stmt = conn.prepare(
...
pub fn delete_character(
    ...
    let (illustration_files, message_ids) =
        purge_thread_data(conn, character_id, &thread_ids, false)?;
...
pub fn clear_chat_history(
    ...
    let (illustration_files, message_ids) =
        purge_thread_data(conn, character_id, &thread_ids, keep_media)?;
