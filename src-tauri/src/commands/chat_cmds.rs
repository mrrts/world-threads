...
pub fn collect_reactions_by_message(
    ...
    let mut by_msg: std::collections::HashMap<String, Vec<Reaction>> =
...
pub fn emit_character_reaction(
    ...
    if emoji.is_empty() {
        log::warn!(
            "[CharReact] skip: emoji is empty (LLM returned nothing and fallback was empty)"
        );
        return Vec::new();
    }
    let Ok(conn) = db.conn.lock() else {
    ...
    match add_reaction(&conn, &r) {
        ...
        Err(e) => {
            log::warn!(
                "[CharReact] add_reaction failed for ({}, {}, 'assistant'): {}",
                target_message_id,
                emoji,
                e
            );
...
pub async fn send_message_cmd(
    ...
    let (
            ...
            match search_messages_fts(&conn, &thread.thread_id, &fts_query, 6) {
                Ok(fts_msgs) => {
                    let fts_msgs: Vec<_> = fts_msgs
                        ...
                        .collect();
                    log::info!(
                        "[Memory] FTS messages: {} results for {:?}",
                        fts_msgs.len(),
                        fts_query
                    );
                    for m in fts_msgs {
        ...
        let narration_tone = get_setting(&conn, &format!("narration_tone.{}", character_id))
            .ok()
            .flatten();
        let leader = get_setting(&conn, &format!("leader.{}", character_id))
            .ok()
            .flatten();
        ...
        let has_chunks =
            {
                ...
                log::info!(
                    "[Memory] Vector store: {} chunks for character {}",
                    count,
                    character.display_name
                );
                count > 0
    ...
    if let Some(ct) =
        build_cross_thread_snippet(&db, &character_id, &thread.thread_id, user_profile.as_ref())
    {
        full_retrieved.push(ct);
    }

    log::info!(
        "[Memory] Total retrieval context: {} items passed to dialogue",
        full_retrieved.len()
    );
    ...
    let latest_reading = {
        let conn = db.conn.lock().map_err(|e| e.to_string())?;
        list_daily_readings(&conn, &character.world_id, 1)
            .unwrap_or_default()
            .into_iter()
            .next()
    };
    ...
    let current_world_day_for_stance: Option<i64> =
        recent_msgs.iter().rev().find_map(|m| m.world_day);
    ...
    let (formula_momentstamp_text, formula_momentstamp_signature): (
        ...
        let prior_sig: Option<String> = {
            let conn = db.conn.lock().map_err(|e| e.to_string())?;
            crate::db::queries::latest_formula_signature(&conn, &thread.thread_id)
                .ok()
                .flatten()
        };
    ...
    let (current_loc, location_pair) = {
        let conn = db.conn.lock().map_err(|e| e.to_string())?;
        let cl = get_thread_location(&conn, &thread.thread_id).ok().flatten();
                    ...
                    if let Some(note) = crate::ai::conscience::build_correction_note(&verdict) {
                        ...
                        .await
                        {
                            Ok((corrected, corrected_usage)) => {
                                log::info!(
                                    "[Conscience] {} reply corrected after drift",
                                    character.display_name
                                );
                            ...
                            Err(e) => {
                                log::warn!(
                                    "[Conscience] regeneration failed, keeping original draft: {e}"
                                );
                            }
                        }
                    }
    ...
    if !is_local {
        ...
        log::info!(
            "[Memory] Generating embeddings for {} items",
            embed_texts.len()
        );
        match orchestrator::generate_embeddings_with_base(
            &model_config.openai_api_base(),
        ...
        .await
        {
            Ok((embeddings, embed_tokens)) => {
                log::info!(
                    "[Memory] Got {} embeddings, storing in vector DB",
                    embeddings.len()
                );
                let conn = db.conn.lock().map_err(|e| e.to_string())?;
                let _ = record_token_usage(
                ...
                for (i, emb) in embeddings.iter().enumerate() {
                    if i < embed_ids.len() {
                        match insert_vector_chunk(
                            ...
                            Ok(_) => {}
                            Err(e) => {
                                log::warn!("[Memory] Failed to store chunk {}: {e}", embed_ids[i])
                            }
                        }
                    }
                }
    ...
    log::info!(
        "[Memory] Maintenance check: should_run={} (interval={})",
        should_run_maintenance,
    ...
    if should_run_maintenance {
        ...
        .await
        {
            Ok((update, usage)) => {
                ...
                log::info!(
                    "Memory maintenance completed for thread {}",
                    thread.thread_id
                );
            }
...
pub fn edit_message_content_cmd(
    ...
    if is_group {
        update_group_message_content(&conn, &message_id, &content, tokens)
            .map_err(|e| e.to_string())
    } else {
...
pub fn delete_message_cmd(
    ...
    conn.execute(
        &format!("DELETE FROM {} WHERE message_id = ?1", fts_table),
        rusqlite::params![message_id],
    )
    .ok();
    conn.execute(
...
pub async fn prompt_character_cmd(
    ...
    if needs_nudge {
        dialogue_msgs.push(Message {
            ...
            role: "user".to_string(),
            content: "[The user looks at you expectantly, waiting for you to say something.]"
                .to_string(),
            tokens_estimate: 0,
            sender_character_id: None,
            created_at: Utc::now().to_rfc3339(),
    ...
    if conscience_enabled {
        ...
        .await
        {
            Ok(verdict) => {
                ...
                if !verdict.passed {
                    log::warn!(
                        "[Conscience] {} (prompt) draft flagged: {:?}",
                        character.display_name,
                        verdict.failures
                    );
                    if let Some(note) = crate::ai::conscience::build_correction_note(&verdict) {
                        ...
                        .await
                        {
                            Ok((corrected, corrected_usage)) => {
                                log::info!(
                                    "[Conscience] {} (prompt) reply corrected after drift",
                                    character.display_name
                                );
                                reply_text = corrected;
                                dialogue_usage = corrected_usage;
                            ...
                            Err(e) => log::warn!(
                                "[Conscience] prompt regeneration failed, keeping original: {e}"
                            ),
                        }
                    }
                } else {
                    log::info!(
                        "[Conscience] {} (prompt) draft passed",
                        character.display_name
                    );
                }
            }
...
pub async fn try_proactive_ping_cmd(
    ...
    let loaded = {
        ...
        let last_user_at: Option<String> = conn
            .query_row(
                ...
                if since < QUIET_AFTER_USER_MSG_SECS {
                    return Ok(ProactivePingResult {
                        ...
                        skipped_reason: Some(format!(
                            "{}s since last user msg (min {}s)",
                            since, QUIET_AFTER_USER_MSG_SECS
                        )),
                    });
                }
            }
            ...
            if let Ok(dt) = chrono::DateTime::parse_from_rfc3339(ts) {
                ...
                if since < required {
                    return Ok(ProactivePingResult {
                        message: None,
                        skipped_reason: Some(format!(
                            "{}s since last ping (rolled min {}s)",
                            since, required
                        )),
                    });
                }
    ...
    let (
        ...
        let recent_msgs = match current_wd {
            Some(day) => {
                ...
                if day_msgs.len() >= 4 {
                    log::info!(
                        "[Dream] scoped to world_day={} ({} messages)",
                        day,
                        day_msgs.len()
                    );
                    day_msgs
                    ...
                    log::info!(
                        "[Dream] world_day={} too thin ({} msgs); falling back to budget window",
                        day,
                        day_msgs.len()
                    );
...
pub async fn generate_narrative_cmd(
    ...
    let (
        ...
        let narration_tone = get_setting(&conn, &format!("narration_tone.{}", character_id))
            .ok()
            .flatten();
        let narration_instructions =
            get_setting(&conn, &format!("narration_instructions.{}", character_id))
                .ok()
                .flatten();
...
pub use crate::commands::illustration_cmds::{
    base64_encode_bytes, png_aspect_ratio, IllustrationResult,
};
...
pub async fn adjust_message_cmd(
    ...
    let (original_msg, character, model_config) = {
        ...
        let char_id = msg.sender_character_id.as_deref().or_else(|| {
            if table == "indiv" {
...
pub async fn reset_to_message_cmd(
    ...
    // at its current state.
    {
        ...
        let conn = db.conn.lock().map_err(|e| e.to_string())?;
        let table_for_anchor = if is_group {
            "group_messages"
        } else {
            "messages"
        };
        let anchor_created_at: Option<String> = conn
            .query_row(
                &format!(
                    "SELECT created_at FROM {} WHERE message_id = ?1",
                    table_for_anchor
                ),
                params![message_id],
                |r| r.get(0),
        ...
        if let Some(anchor_ts) = anchor_created_at {
            let target_char_ids: Vec<String> = if is_group {
                let char_ids_json: Option<String> = conn
                    .query_row(
            ...
            .await
            {
                Ok((update, usage)) => {
                    ...
                    if let Some(new_summary) =
                        update.get("updated_summary").and_then(|v| v.as_str())
                    {
                        let conn = db.conn.lock().map_err(|e| e.to_string())?;
                        let artifact = MemoryArtifact {
    ...
    if anchor_role == "user" && !is_group {
        let (
            ...
            if !fts_query.is_empty() {
                if let Ok(fts_msgs) = search_messages_fts(&conn, &thread_id, &fts_query, 6) {
                    for m in fts_msgs.into_iter().filter(|m| {
                        m.role != "illustration"
                            && m.role != "video"
                            && m.role != "inventory_update"
                    }) {
        ...
        let reactions_by_msg = collect_reactions_by_message(&db, &recent_msgs);
        let mut retrieved = retrieved;
        if let Some(ct) = build_cross_thread_snippet(
            &db,
        ...
        if conscience_enabled {
            ...
            .await
            {
                Ok(verdict) => {
                    ...
                    if !verdict.passed {
                        log::warn!(
                            "[Conscience] {} (reset) draft flagged: {:?}",
                            character.display_name,
                            verdict.failures
                        );
                        if let Some(note) = crate::ai::conscience::build_correction_note(&verdict) {
                            ...
                            .await
                            {
                                Ok((corrected, corrected_usage)) => {
                                    log::info!(
                                        "[Conscience] {} (reset) reply corrected after drift",
                                        character.display_name
                                    );
                                    reply_text = corrected;
                                    dialogue_usage = corrected_usage;
                                ...
                                Err(e) => log::warn!(
                                    "[Conscience] reset regeneration failed, keeping original: {e}"
                                ),
                            }
                        }
                    } else {
                        log::info!(
                            "[Conscience] {} (reset) draft passed",
                            character.display_name
                        );
                    }
                }
