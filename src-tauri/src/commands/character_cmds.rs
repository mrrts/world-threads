...
#[tauri::command]
pub fn list_archived_characters_cmd(
    db: State<Database>,
    world_id: String,
) -> Result<Vec<Character>, String> {
    let conn = db.conn.lock().map_err(|e| e.to_string())?;
...
#[tauri::command]
pub fn get_character_derivation_cmd(
    db: State<Database>,
    character_id: String,
...
pub async fn regenerate_character_derivation_cmd(
    ...
    let (prompt, base_url, model) = {
        let conn = db.conn.lock().map_err(|e| e.to_string())?;
        let _ =
            get_character(&conn, &character_id).map_err(|e| format!("character not found: {e}"))?;
...
pub fn delete_character_cmd(
    ...
    let (illustration_files, message_ids) =
...
pub fn clear_chat_history_cmd(
    ...
    let conn = db.conn.lock().map_err(|e| e.to_string())?;
    let (illustration_files, message_ids) =
        clear_chat_history(&conn, &character_id, keep_media).map_err(|e| e.to_string())?;
