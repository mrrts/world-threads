...
```bash
.agents/skills/mission-arc/render.sh
```
...
.agents/skills/mission-arc/render.sh 50

# Filter by collaborator (Codex vs. Claude). All commits are authored by
...
.agents/skills/mission-arc/render.sh 30 --invert-grep --grep "Co-Authored-By: Claude"  # Codex commits
.agents/skills/mission-arc/render.sh 30 --grep "Co-Authored-By: Claude"             # Claude commits

# Filter by date:
.agents/skills/mission-arc/render.sh 50 --since "1 week ago"
.agents/skills/mission-arc/render.sh 30 --since "2026-04-25"
...
- **`/take-note`** — when Ryan describes a lived in-app observation, the
  ...
  reference the right recent commit.
...
- **`/derive-and-test` / `/rule-arc`** — bite-test arcs are calibrated by
  whether the rule under test connects to live arc-themes.

## Mirror — Claude surface

Per the parity-defaults doctrine (AGENTS.md / CLAUDE.md "Earning the
departure from a default — three polarities" section), this skill is
mirrored at `.claude/skills/mission-arc/` with identical render.sh +
...
existing AGENTS.md doctrine; mission-arc just makes them queryable as a
