...
**Same law, substrate-native encodings — not naive string parity across generative paths.** The paragraph above is one moral law; it does not require pasting the same system instruction into every `build_*_system_prompt`. Dialogue (`build_solo_dialogue_system_prompt`, `build_group_dialogue_system_prompt`, proactive pings that wrap solo) may carry an explicit system-private hidden-motive line because the model answers **in character** and benefits from a crisp steer against compliment-theatre. Dream generation (`build_dream_system_prompt`) should **not** copy that line verbatim: `dream_craft_block`'s *sideways, never direct; transformation, not explanation* is already how "inhabited, not announced" expresses when the substrate is **subconscious compression**, not turn-taking speech — importing dialogue-shaped analytic curiosity would fight the dream contract. Immersive narrative (`build_narrative_system_prompt`) is external second-person narrator toward the user; character-motive idiom is the wrong shape there too (the load is POV and beat, not "your hidden motive as this character"). This composes with the deeper parity rule: **parity of law, not parity of identical prompt text.** Before extending user-curiosity machinery to a new substrate, classify it as character-speech vs dream-compression vs narrator-to-user — then encode in that register or rely on existing craft when it already carries the law. Surfaced 2026-04-28 by /eureka cross-reading dialogue, dream, and narrative builders after the hidden-motive dialogue ship.
