...
**Tuning-frames belong at position-0 — same-shape claim implies same-position placement.** Surfaced 2026-05-04 by /eureka focused on the prompt stack. The Mission Formula and 𝓕_Ryan are injected at position-0 of every LLM call (`inject_mission_formula` / `inject_ryan_formula` in `openai.rs`); the IDENTITY-block code comment for `character.derived_formula` explicitly claims it has *"the same prefix-sentence shape as the MISSION FORMULA: not-a-directive-to-compute, but the register this character is held under."* Same-shape claim, but the character formula was originally buried in the IDENTITY block (mid-prompt) rather than at position-0 — architectural intent and placement diverged. Discovery: tuning-frames whose code comment claims same-shape parity with the Mission Formula belong at the same position. The character formula has a feature-scoped invariant `CHARACTER_FORMULA_INVARIANT_FRAMING` (in `prompts.rs`) + `wrap_character_formula_invariant` helper + a top-of-stack injection in `orchestrator::run_dialogue_with_base`. **Default-on as of 2026-05-05 per Ryan's directive (re-affirmed 2026-05-07; no flag, no env gate)** — the elevation was originally env-flag-gated under `CHARACTER_FORMULA_AT_TOP=1` awaiting a paired-probe bite-test (Aaron + Pastor Rick × ELEVATED vs CURRENT × N=5 within-cell × 2 characters), but Ryan promoted to default-on and removed the flag without the bite-test landing. The pattern generalizes: when a code comment claims same-shape parity with an established top-of-stack invariant, the placement should match the claim; the originally-specified bite-test path (paired-probe across two stable-distinctive characters) stands as the methodology of record for future tuning-frame elevations when a founding-author directive does not override.
