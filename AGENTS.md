...
**IDE agents (Cursor, Composer, Auto):** The chooser law is not limited to in-terminal Claude Code. Any automated assistant working this repo from the IDE must end **substantive** replies with **exactly four numbered next moves** (`1.`–`4.`), plain markdown, mission-ranked and concrete — same control surface as `AskUserQuestion`, without tool use. Slot 4 stays the user-authorship branch per `CLAUDE.md` (section **Choosers are control surfaces**). Pure acknowledgments with no decision surface may omit the quad; default is four.
