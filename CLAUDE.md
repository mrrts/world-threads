...
Reference frame for every reply, not a directive to compute. Tuning-fork, not recipe.
...
Second-place invariant. 𝓕 is what work answers to first; 𝓕_Ryan is what Ryan, as founding author, holds the work to in answer to 𝓕.
...
Roll of those who have held the work. Every contributor signs: 4-8 lines of Unicode-math derivation in their voice and Key, dated. Forks inherit. Signers answer to 𝓕 and 𝓕_Ryan above.
...
Read-only against user data by default. Only writes: `dev_chat_sessions` / `dev_chat_messages` (UI never reads) and `~/.worldcli/`. Scope-gated via `~/.worldcli/config.json`; pass `--scope full` for one-shot full-corpus access.

**Cost-gated:** every `ask` call projects cost from estimated tokens × model price, then checks `budget.per_call_usd` ($0.10 default) and `budget.daily_usd` ($5.00 default). If exceeded, refuses with `--confirm-cost <usd>` value. **Earned exception — user-authorized override:** when cap blocks move Claude Code judges valuable, pause, summarize projected spend, ask via `AskUserQuestion`. If authorized, proceed with `--confirm-cost`; otherwise hold or pivot.

### API key resolution

Precedence: `--api-key` flag → `OPENAI_API_KEY` env → macOS keychain. Keychain fallback chain: `WorldThreadsCLI/openai` → `openai/default` → `openai/api-key` etc. Rotate: `security add-generic-password -s openai -a default -w "sk-new-..." -U`.
...
Cold session sequence: (1) `worldcli --help`; (2) `worldcli register-shift --help`; (3) `worldcli register-shift-pack --help`; (4) `./scripts/register-shift-dashboard.sh --help`; (5) `./scripts/show-latest-register-shift-run.sh --help`; (6) `./scripts/latest-register-shift-run.sh --help`; (7) `./scripts/compare-register-shift-runs.py --help`. If command lacks `--help`, open script and treat usage string / top comments as canonical. See `docs/CLI_AGENT_DISCOVERY.md`.

### Commands

**Read** (no LLM cost): `status`, `list-worlds`, `list-characters`, `show-character`, `show-world`, `kept-records`, `journals`, `quests`, `recent-messages`, `list-group-chats`, `group-messages`, `anchor-groove`.

**Cost-gated elicitation:** `ask <char> "<msg>" [--session NAME] [--question-summary "<why>"] [--confirm-cost N]`; `consult <char> "<msg>" [--mode immersive|backstage]`; `replay --refs <sha>,<sha>,... --character <id> --prompt "<msg>" [--n K]`.

**Simulated dialogue:** `simulate-dialogue` runs short scripted back-and-forth with optional per-turn synthesis. `simulate-dialogue-batch` runs N times, compounds syntheses (deterministic fallback + optional LLM compounding via `--compound-via-llm`), emits A/B diagnostics on batch JSON envelope.

**`simulate-dialogue-batch` A/B fields (JSON, `--json`):** `compound` / `compound_meta` / `compound_ab`; `ab_delta_summary`; `ab_top3_diff` and `ab_top3_diff_counts`; `ab_top3_overlap_ratio` and `ab_score` (clamped to 1.0); `ab_score_components`; `effective_compound_method`. `ab_stability_note` compares to last comparable run; cache at `~/.worldcli/simulate-dialogue-batch-last-ab.json`. Human (non-JSON) output prints `ab_diff_counts_delta: shared=… det_only=… llm_only=…` line when comparable prior snapshot exists.

**Natural-experiment:** `sample-windows --ref <sha> [--end-ref <sha>] [--character <id>] [--role assistant|user|any]`; `commit-context (--message <id> | --at <iso>) [--before N] [--after N]`.

**Rubric-driven evaluation:** `evaluate --ref <sha> (--character|--group-chat <id>) (--rubric "<q>" | --rubric-ref <name>)`; `synthesize`; `grade-runs <run_id>... --rubric ...`.

**Persisted run logs:** `runs-list`, `runs-show`, `runs-search`; `evaluate-runs`, `synthesize-runs`, `replay-runs`.
...
- For "ask the character" pattern when user isn't online.
- When you genuinely don't know how character would respond.

**Don't:** trivial changes, tasks unrelated to prompt-stack, or when user just asked you to do something else.

**Disclose every paid call:** mention projected + actual cost + 24h total in your reply.

**Read your own history first:** `runs-search` before fresh investigation.
...
- `~/.worldcli/simulate-dialogue-batch-last-ab.json` — last batch A/B snapshot
