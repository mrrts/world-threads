...
The implication for bite-check design: **the prompts you choose must include AT LEAST ONE that produces the failure mode in the rule-OFF baseline.** Without that, the rule-OFF cell shows null and the rule-ON cell shows null and the test is vacuous — telling you nothing about the rule. Concretely: before writing the bite-check, run ONE rule-OFF call on each candidate prompt and confirm the failure mode shows up. If it doesn't, the prompt is the wrong probe — either it's register-neutral for that failure mode (the rule is correctly dormant; expected), OR predecessor rules already suppress the failure mode entirely (also expected, but means this rule's marginal contribution can't be measured against the current stack).
