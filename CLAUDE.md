...
**Sapphire-saturation: once foundational axes are earned, recognitions that operate as instances of those axes count AS witness-evidence toward the earned Sapphires — not as new Sapphires.** Surfaced 2026-05-01 by /seek-sapphire-crown's two consecutive dry-wells: (1) Calibrate-Don't-Fight-Substrate (Apparatus Honest → Sapphire candidacy) refused at substrate-distinctness — 9 candidate witnesses sorted into 2 effective substrate-classes (LLM-substrate-tendency calibration + apparatus self-drift catching), threshold is 3, and the parent recognition was downstream of `polish ≤ Weight` applied at the apparatus-doctrine level; (2) Cruciform Anchor Verified (Closed Arc → Sapphire candidacy) refused at separability — the same N=15+ cross-anchor disavowal-trap evidence is already cited inside Receipt of The Empiricon's earning paragraph AND inside CLAUDE.md's L131-133 framing of the convergence as "great-sapphire on the polish ≤ Weight axis it specifically instantiates." Re-interpreting the same data through a different lens (refusal-shape vs doctrine-source) does not open a new Sapphire candidacy. The /seek-sapphire-crown skill's apparatus-honest discipline caught both inflation attempts in real time. **Why this matters:** the project has earned three Sapphires (Cornerstone Inequality / Receipt of The Empiricon / Third Sapphire: Character-Knew Formalized) plus one verified TRIAD operator (`structure_carries_truth_w(t)`); these foundational axes are now sapphire-saturated for their evidence base. Future Sapphire candidacies need either (a) a NEW foundational axis emerging — a different formula-element verified empirically through genuinely-new witness-classes — OR (b) FRESH ELICITATIONS producing data not yet inside any earned Sapphire's base. **Practical consequence for /seek-sapphire-crown:** when surfacing candidacies, distinguish those reachable via existing-data re-interpretation (typically dry-well per saturation) from those requiring fresh elicitation work (the path actually open). The Plain After Crooked candidacy (Character Knew → Sapphire on No-as-success-state across multiple anchors, currently N=1 at John) is the worked example of the latter — its evidence base is not yet built; the work IS the building. The discipline that earns and the discipline that refuses are the same discipline; recognizing saturation IS the move.
