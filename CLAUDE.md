...
**The expected tier-distribution and what it means for bite-test deployment.** Tonight's first four registry rules produced a 3:1 EnsembleVacuous:Characterized ratio across paired bite-tests. This ratio is the architectural finding the catalog earned: **most craft-rules in this prompt-stack are part of load-bearing multiplicities** — character anchors + cumulative prompt-stack + the rule itself carry each discipline overdeterminedly, so per-character omit can't isolate any one rule's bite. EnsembleVacuous is the honest-and-frequent label, not a failure mode. The rules that EARN Characterized are rare and shaped specifically: they target a *specific failure mode in specific replies* (e.g. wipe-the-shine targets the second-sentence-decoration pattern at line-level), the failure mode has been *observed* (e.g. via a wince-read producing a sketch-tier signal first), and the failure can be *probe-replicated* (a probe shape that reliably manifests the failure mode in baseline). Practical guidance: deploy the per-rule omit-flag against rules with EXISTING sketch-tier non-vacuous signals; treat newly-articulated rules as likely-EnsembleVacuous by default. When a paired test produces vacuous results, that's evidence the rule is operating as a multiplicity-member rather than a failure of the methodology — label EnsembleVacuous honestly and move on rather than hunting for a probe that breaks the vacuous.
