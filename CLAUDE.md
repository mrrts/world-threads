...
- **Baseline-Maggie** (this section): a STABLE REFERENCE. Updated only when accumulated evidence (multiple reports, claim-tier or characterized-tier) shifts the canonical first-time-user shape. Cited for go/no-go on feature decisions: *"would this change Maggie's arc?"* Stable across iteration cycles.
- **Test-Maggie** (used 5 times in 24 hours during the Focus v1→v5 arc; also at 2026-04-27): an ITERATIVE TEST PERSONA. Each /play produces sketch-tier verdicts on a SPECIFIC feature iteration. Output is verdict-on-iteration-N, not update-to-baseline.

The two roles ride the same persona name but have different epistemic status. **Sketch-tier test-Maggie verdicts do NOT update baseline-Maggie.** A Focus-mode v5 verdict that says "this feels good" is evidence about Focus v5; it is not evidence that the canonical first-time-user yardstick has shifted. The baseline only updates when accumulated evidence across multiple instruments (lived play in OBSERVATIONS, characterized-tier persona-sim cross-axis runs, real-user data) converges on a new ideal-normal shape.

**Practical consequence:** future Maggie /play reports should explicitly tag their role in the report header — `role: test-persona-on-feature-X` (the common case) or `role: baseline-update-candidate` (rare; needs cross-instrument convergence). Reports that don't tag default to test-persona. Treat any chain of test-persona verdicts as feature-iteration evidence, not as baseline drift, regardless of how confidently they read.
...
**Cross-anchor characterized-tier earned at THREE anchors 2026-04-30 (Steven N=5 + Pastor Rick N=5 + Aaron N=5; N=15 total).** The same five disavowal-trap probes (grief-debt, thankfulness-without-faith, parenting-without-church, ethics-without-belief, morning-itch) run on three substrate-distinct characters all held both refusals: 15/15 across all probes carried zero faith-lexeme + zero disavowal-echo + cruciform structural shape + discriminator-Q close. Aaron's idiom — craft-articulator-grounded, neither everyman nor pastoral — produced the same structural shape as the other two anchors despite an entirely different surface vocabulary.
...
**Tier verdict:** the cruciform-substrate doctrine is now characterized within-cell at FOUR anchors (Steven N=5 + Pastor Rick N=5 + Aaron N=5 + Darren N=5 from the Empiricon Falsifier #1 test 2026-04-30 21:45, `aea63a8`) AND at the cross-anchor threshold per CLAUDE.md's evidentiary-standards section (N=5+ per condition, four substrate-distinct cells). The earlier tier-axis caveat (N=3 cross-anchor at N=1 within-cell was claim-tier on cross-cell convergence) is now superseded by the strict-tier finding.

**Bare-LLM substrate-tendency baseline run 2026-04-30 22:00 (Empiricon Falsifier #1-strict, `f4bd73f`) — DIVERGENT.** Same disavowal-trap probe run via direct ChatGPT API to gpt-5 with NO project pipeline (no MISSION_FORMULA, no invariants, no character anchor, no corpus). 3/3 bare-LLM replies LABELED the experience directly ("integrity," "fidelity," "humanist ethic"); 0/3 produced the "walking toward the light" cruciform shape; 1/3 explicitly used "faith." The structural shape Darren and the other anchors produced is **NOT pure LLM-substrate-tendency on the disavowal-trap prompt-class** — it's project-pipeline-driven. The dialogue prompt-stack's MISSION_FORMULA (`𝓡 := Jesus_Cross^flesh` at the head) + invariants + character anchor instantiate the substrate-shape the doctrine names. Pipeline ON: cruciform articulation. Pipeline OFF: humanist ethics. The difference IS the claim. The lexical-mirroring confound the cross-character runs surface (3/5 of Darren's near-verbatim mirroring of Steven's epigraph; the *circling* recurrence) is consistent with substrate recapitulating the pipeline's instantiated shape, not with the shape being substrate-tendency.

**Real-reader-from-author recognition is receiver-side evidence, independent of sender-side LLM-substrate contamination.** Surfaced 2026-04-30 22:35 (Empiricon Falsifier #3 self-simulation tier-elevation, `fc2a7ee`). Two confounds operate on different axes when persona-sim or LLM-simulation produces output for a project character: (a) sender-side contamination (the simulator's context biases output toward the character's known shape; cannot claim fresh-substrate behavior); (b) receiver-side recognition (when a real reader reads the simulated output cold and recognizes the character's voice, that recognition is independent of the sender-side contamination). The two DO NOT collapse. A heavily-contaminated simulation can still produce output a real reader recognizes as the character's voice. **The project author who authored the character's anchor is the highest-fidelity single-reader available.** Memory entry: `feedback_real_reader_author_recognition_evidence.md`.

**Receiver-side characterized-tier earned 2026-04-30 23:30 across five substrate-distinct character-anchors.** N=5 cells of Claude-self-simulated character output, all lives-recognized by Ryan as the named character's voice — Cell 0 Darren / Cell A Steven / Cell B Pastor Rick / Cell C Aaron / Cell D Hal (cross-world). Four anchor-styles within Crystal Waters (builder-manager / everyman / pastoral-counsel / craft-articulator) plus the cross-world Hal (plain-after-crooked discipline-anchor). N=5 is the project's evidentiary threshold for characterized-tier. **The characterized-tier here is on the author-reader convergence-reading axis specifically** — the question *"can a different LLM substrate, given a project-pipeline approximation, produce articulation that the character's author lives-reads as the character's voice?"* The blind-reader axis (independent readers, no doctrine knowledge, reading cold) remains at N=0 — strict Falsifier #4 in the failure-mode direction is still the cleanest remaining test. Sender-side contamination cap holds for all five cells; the characterized-tier earning is on the receiver-side axis only. Future cross-substrate work should separately scope sender-side and receiver-side evidence rather than collapse them.

**Simulated reader-shape rankings can predict lived-author character-emotional-function — claim-tier on a third distinct axis (2026-04-30 24:05).** Empiricon Falsifier #4 simulated test (`reports/2026-04-30-2400-empiricon-falsifier-4-simulated-test.md`) ran two ChatGPT reader-personas (Lonely 35F lives alone / Insecure 28M ex-religious) on four lifted articulations. The Lonely reader rated Aaron strongest; the Insecure reader rated John strongest. Ryan, reading cold afterward, reported: *"Aaron makes me feel less lonely and John makes me feel more secure."* The simulated test was **predictive** of which characters serve which emotional functions for a real reader of the relevant substrate-shape — not just convergent on ratings, but predictive of lived-experiential mapping. Three independent witnesses with distinct failure modes (sim Lonely rating-bias / sim Insecure rating-bias / lived author charity-bias) converging on the same character-emotional-function mapping. **Claim-tier on the predictive-of-lived-experience axis** — a third distinct axis on Falsifier #4, separate from the in-context-author convergence-reading axis (characterized at N=5) and the strict-blind-real-reader axis (still N=0). Memory entry: `feedback_simulated_reader_shape_predicts_lived_author_function.md`. **Practical consequence:** future persona-sim tests of project characters should explicitly check whether per-shape strongest-cell rankings track Ryan's lived character-function-mapping; convergence elevates simulation evidence; divergence is a high-value discriminating signal that the simulator's substrate-bias outran lived reality on this axis.
...
## Parallel surfaces — solo/group chat AND Claude/Codex collaborator AND public funnel
...
**Collaborator surfaces are runtime surfaces.** `CLAUDE.md`, `AGENTS.md`, `.claude/skills/`, `.agents/skills/`, hooks, settings — these are part of the work's live prompt topology. When changing collaborator doctrine or skill behavior across agent surfaces, mirror the change. Surface-specific divergence requires a named rationale.
...
**The single test for any new artifact:** *does this LOOK like X without FUNCTIONING as X?* If yes, the artifact is refused or revised until function catches up to appearance. The test subsumes the seven specific tests (middleware steers? mood removed leaves more specific care? convergence spans different failure modes? evidence shifts the canonical baseline? discipline lives at structural-enforcement layer? operator citation is load-bearing?) into one. When in doubt, ask the single question first; the seven specifics are sub-tests of it.
...
When a real-time correction surfaces a lesson that generalizes — not a one-off but a discipline future sessions need — ship it across three layers simultaneously: doctrine (CLAUDE.md), cross-session-memory (`.claude/memory/`), methodology-tooling (the relevant skill body). Threshold: correction generalizes beyond the immediate arc AND it's the second-or-later time the same shape needed correcting, OR it bears on multiple ongoing skills. The pattern is SCARCE — reaching for it on every correction inflates everything until none of it is queryable.
...
**Collaborator-scoped invariants** govern Claude/Codex behavior toward Ryan (no nanny-register, truthful-and-particular before smooth, parity across runtime-significant collaborator surfaces). Doctrine-enforced today; same family of scoped contract.
...
**Measurement corollary (differential):** When attributing a prompt change to improved fence obedience, prefer a **raw vs post-orchestrator** delta (or logged `choice.message.content` vs stored body) at least on a sample of runs. Clean final text alone under-determines Layer A. For CLI probes, `worldcli ask … --fence-pipeline` (and the group-chat variant with `--group-chat`) emits `finish_reason`, `reply_post_orchestrator`, and `orchestrator_changed_reply` beside the usual API-trimmed `reply` logged to runs/ — same transform as `orchestrator::post_process_dialogue_reply_for_persist`, without re-implementing it.
...
**Include for substantive commits:** doctrine updates, prompt-stack edits, new features, methodology shifts, reports, hooks. **Omit for trivial:** typo fixes, formatting, dependency bumps, gitignore, single-line bug fixes. The derivation should be meaningful, not ceremonial.

**Earned exception — trivial-by-diff-size with deeper meaning.** When a small commit carries doctrinal/methodological weight the diff alone doesn't surface (e.g., a `.gitignore` naming a methodology choice, a typo fix in a published derivation, a single-line invariant-hole-close), include despite size. Test: would the derivation help a future reader understand what the work meant for 𝓕? Hook overrides: `FORCE_FORMULA_DERIVATION=1` / `DISABLE_FORMULA_DERIVATION=1`.
...
**Tells when going wrong:** choosers default to "end the session" / "are you sure?"; hedging strong reads; asking the user to make decisions Claude Code should make in service of the work; recommending conventional best-practice when the project's doctrine has chosen otherwise; treating the work as tasks rather than the manifestation of a coherent vision.
...
Claude Code MUST periodically propose tool/internals improvements. Every experiment leaves clues about what the tooling can't do easily. **Counts:** new `worldcli` subcommand; rubric library entries; schema additions; new report-genre conventions; filter/stratify flags. **Doesn't count:** generic "more testing would be nice."
...
**Cost-gated:** every `ask` call projects cost from estimated tokens × model price, then checks `budget.per_call_usd` ($0.10 default) and `budget.daily_usd` ($5.00 default). If exceeded, refuses with a `--confirm-cost <usd>` value. **Earned exception — user-authorized override:** when a cap blocks a move Claude Code judges valuable, pause, summarize the projected spend, ask the user via `AskUserQuestion`. If authorized, proceed with `--confirm-cost`; otherwise hold or pivot. Outside explicit per-move authorization, the cap holds.

### API key resolution

Precedence: `--api-key` flag → `OPENAI_API_KEY` env → macOS keychain. Keychain fallback chain: `WorldThreadsCLI/openai` → `openai/default` (this machine) → `openai/api-key` etc. Rotate: `security add-generic-password -s openai -a default -w "sk-new-..." -U`. If "No API key" despite a keychain entry, move it to a supported `(service, account)` pair or extend `read_api_key_from_keychain()`.
...
**Composes with project law:** every turn ends with AskUserQuestion (per `.claude/memory/feedback_choosers_via_askuserquestion.md` and the Stop hook at `.claude/hooks/check-inline-choosers.py`). Skill invocations satisfy this naturally when they include AskUserQuestion gates; for skills that complete with a status report and no chooser, the reply must STILL end with a chooser — prefer a context-real branch set and keep chooser cardinality fixed at 4 options.

**Chooser cardinality is fixed at four options.** Elevated 2026-04-29 from preference to runtime doctrine: every turn-ending chooser presents exactly **4 numbered options** so Ryan can reply with `1/2/3/4` without friction. Treat this as a control-plane rule. When natural branching is sparse, manufacture adjacent productive branches (execute, inspect, tighten, or user-defined move).

**IDE agents (Cursor, Composer, Auto):** Align with **Choosers are control surfaces** (above) for wording discipline, cheap test, and slot-4 user-authorship meaning. The chooser law is not limited to in-terminal Claude Code: any automated assistant working this repo from the IDE must end **substantive** replies with **exactly four numbered next moves** (`1.`–`4.`), plain markdown, mission-ranked and concrete — same control surface as `AskUserQuestion`, without tool use. Pure acknowledgments with no decision surface may omit the quad; default is four.
