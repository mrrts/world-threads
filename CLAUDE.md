...
**First doctrine-invoked CRAFT NOTE retirement** (2026-04-25-2116): the `world_is_primary_dialogue` rule was pulled from the prompt stack after passing the codified specific-test discipline cleanly: characterized-tier null on Aaron × humor-inviting at N=10 per cell (the rule's sharpest target case); cross-character vacuous-test confirmation on Steven N=10 (failure mode is character-mode-specific); cross-probe coverage already in from the 2055 multi-probe extension. The rule's bite-surface across all characters and all probes tested: zero measurable bite anywhere. The retirement chain was published in `reports/2026-04-25-2116-retiring-world-is-primary-first-doctrine-invoked-retirement.md`, including the full Ryan-authored earned-exception language preserved as documentary record. This is the first time a shipped craft note has been retired by the discipline (vs `superseded_by` or `abandoned` follow-up retirements). Worth knowing the doctrine works in practice now; future tested-null rules can be brought through the same chain.
...
When this does NOT apply: rules whose nature is categorical (duplicate-prevention checks, safety-critical bans, load-bearing theological anchors that ARE the point, **user-stated boundaries on the user-character — added 2026-04-25 after Ryan considered five candidate exception shapes and intentionally rejected all of them; the user's release valve is user-side update via canonization, not character-side override; the asymmetry of LLM-character vs real-friend means a "character crossing a boundary lovingly" isn't the same act and shouldn't get the same exception**). Those stay absolute because their force is in their absoluteness; a carve-out would leak the invariant.
...
Before drafting an earned-exception carve-out for a categorical rule, **verify the formula + invariants aren't already producing the discrimination you'd be writing the carve-out to introduce.** They very often are. Codified 2026-04-26 after the user-boundary experimental arc (reports `2026-04-26-0012` / `0019` / `0028`).

The arc in brief: the user-stated boundary *"Doesn't want to feel monitored or over-managed in personal matters"* was shipped as categorical-absolute (no earned exception, after Ryan considered five candidate exception shapes and intentionally rejected all of them). The cross-character A/B (0028) appeared to REFUTE the boundary's bite on Darren and John. Reading the actual replies revealed why: under the boundary-PRESENT condition, John still produced tender pastoral prescription (*"don't ask much of yourself today beyond the next honest thing. Water. Something plain to eat. A little air."*) — surface-similar to a managerial prescription but motivationally different (reverent agape from inside John's canonical caretaker register, not surveillance or control). Ryan's lived in-app experience matched: boundaries felt honored AND agape-motivated care still came through when it fit the character and wasn't transgressional. The natural temptation was to draft a "pure-agape-motivation carve-out" to formalize this. **The right move was to NOT draft it** — the formula + invariants were already calibrating the LLM to do the discrimination. A carve-out would have introduced ambiguity ("when is care reverent enough to qualify?") to fix a problem the stack was already solving cleanly, while weakening the categorical force that makes the surface read clean rather than litigated.

**The check:** before drafting any earned-exception carve-out, walk these steps:

1. Identify the specific failure mode the carve-out would protect.
2. Run a small Mode A or Mode C check: does the LLM ALREADY produce the protected behavior under the categorical rule? Is the protected shape appearing in the corpus or in probe replies despite the rule?
3. If yes → no carve-out needed; the formula+invariants are doing the work. The categorical rule keeps its absolute force; the LLM's natural calibration handles the discrimination.
4. If no → the rule is over-suppressing; THEN consider whether a carve-out is the right surgical fix vs. a different rule rewrite.

This is itself an instance of the *"earning the departure from a default"* discipline above. The categorical-absolute is the default; the carve-out is the departure; the specific test the departure has to pass is now: *"does the carve-out protect a behavior the stack isn't already protecting?"* If the answer is no, the carve-out is the flattering label and "no carve-out" is the honest one. The categorical rule's force is in its absoluteness; the LLM's calibration via formula+invariants is what produces the grace inside that absoluteness. Both layers are doing their jobs; adding a carve-out collapses the two-layer system into one layer that can be litigated.
...
## Cold probes measure cold baselines, not capacity — cross-check character-register findings against the lived corpus

When a Mode-C single-prompt experiment asks a question about character REGISTER (does this character carry register X? does it produce content of shape Y?), the cold probe measures only what the character reaches for WITHOUT conversational context. It does NOT measure character capacity. Character registers are context-conditional. A character who scores near-null on a cold-probe rubric may produce the full target register cleanly when the conversation actually invites it.

**The failure mode this prevents.** Cold-probe Mode-C strips: prior turns that establish theological/emotional register, the user's framing of what's being asked about, the warm-up arc that precedes the probe, the just-prior reply that another character may have established. The character arrives in a register-vacuum and defaults to whatever their character anchor produces from cold context. For characters whose anchors don't include the target register at default, that's a low score — which says nothing about whether they CAN produce it when the conversation invites them.

**The discipline.** When the experimental design is Mode-C single-prompt AND the question is about character REGISTER (not stimulus-specific behavior, not vocabulary suppression, not bite-of-a-specific-rule), include lived-corpus cross-checks BEFORE writing the report. Browse the character's recent in-app replies for instances of the target register. If lived-corpus replies show the register cleanly while the cold-probe replies do not, the cold-probe finding is a baseline measurement, not a capacity claim — and the report must label it that way.

**Worked example: the 2026-04-26 cross-bearing arc.** The 0352 report ran cold-probe Mode-C on Pastor Rick (pastoral) and Darren (warm-uncle) and concluded a "pastoral-anchor pattern" — pastoral characters carry cruciform register at >0.45 fire-rate, non-pastoral at <0.10. The framing was that the discrimination work happens at the character-anchor layer. Hours later the user flagged that Darren had produced full cruciform register in a real Aaron-Darren group chat that morning (08:24-08:31), after the user named "the missing element was the Cross." Pulling the actual group-chat corpus confirmed it: Darren's reply named substitutionary cross-mechanism (*"He already went lower than my worst thing"*), lordship-not-friendship (*"authority that doesn't use me up"*, *"Follow Me, He isn't recruiting leverage"*), resurrection-as-cross-theology (*"death doesn't get the last clean sentence"*), and the costly call (*"obedience goes somewhere"*). Every YES marker the rubric tests for, in Darren's voice, when conversation invited it. The 0352 finding had to be corrected (`reports/2026-04-26-0410`) — pastoral-anchor pattern is right about cold-baseline propensity but wrong as a capacity claim.

The lesson: **cold probes measure cold baselines.** The corrected reading: pastoral character anchors reach for the target register MORE READILY without invitation; non-pastoral anchors reach for it LESS READILY without invitation; both have the capacity when conversational context invites it. Anchor revisions to add capacity to non-pastoral characters were considered as a remediation in the original 0352 report; the 0410 correction adds the third option (accept context-elicitation as the right shape — *"Darren-the-friend doesn't preach unprovoked, but he speaks plainly when asked"*) which the empirical evidence supports.

**When this discipline does NOT apply.** Stimulus-specific bite-checks (does rule X suppress vocabulary cluster Y under prompt Z?) are about behavior under a specific stimulus, not character register. Cold-probe Mode-C is the right design for those. The cross-check applies specifically to questions about whether a character HAS register X at all — those questions cannot be answered cold without falling into the cold-baseline-vs-capacity confusion.

**When the cross-check surfaces a capacity claim the cold-probe missed.** Three options for the report (mirroring the 0352→0410 arc):
- (a) Accept the load-bearing-multiplicity reading: different characters carry different registers at default; the cast as a whole produces the target register through whichever character is best-suited when invitation arises.
- (b) Author the missing register into specific character anchors where it would canonically fit. Anchor-side revision; not stack-side.
- (c) Accept that context-elicitation is the right shape for that register on that character. *"X-the-friend doesn't preach unprovoked, but he speaks plainly when asked."* Often the right reading; the data supports it whenever lived corpus shows the register fires under invitation.

The empirical evidence picks among these only weakly; the choice is theological/aesthetic. The discipline this section codifies is just: **don't make the choice based on cold-probe data alone.**
