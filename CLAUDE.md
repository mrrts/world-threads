...
**Rubric design is load-bearing — a separate discipline from running the bite-test.** The 2026-04-25 arc produced three worked examples of rubric calibration reversing a finding:

- **1711 gentle-release first rubric** keyed on "behavioral abstractions" (brief, compassionate, no extending) and counted in-register closing imagery ("Night's waiting for both of us") as "extending." Reversed the verdict. Tighter rubric keyed on specific failure-mode phrases and explicitly excluded closing-imagery → clean 1.00 → 0.00 signal.
- **1759 verdict-register rubric** keyed on stereotyped opener vocabulary ("Yeah/No/Good") and graded textured verdicts ("I don't trust ducks," "A grim app is already half-broken," "That's your people") as `no`. Reported delta +0.05 (refuted). Tighter rubric keyed on "does the first sentence stake a position regardless of how textured" → re-grade showed delta +0.30 (confirmed).
- **1759 Aaron gentle-release rubric** was single-dimension (binary return-prescription present?) and missed the 4-dimension bite (compression, performed-warmth suppression, scene-prop suppression, second-thought suppression). Multi-dimensional rubric would have caught the partial-bite story.
